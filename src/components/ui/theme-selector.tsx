...
export function ThemeSelector() {
  ...
  return (
    <div className="flex justify-center items-center gap-2">
      ...
      <div className="relative flex items-center rounded-full bg-gray-200 dark:bg-zinc-800 w-36 h-8">
        {/* Option labels */}
        <label
          htmlFor="light"
          className={`absolute left-0 top-0 bottom-0 flex items-center justify-center w-1/3 text-xs font-medium cursor-pointer transition-all duration-300 rounded-full ${
            theme === 'light'
              ? 'bg-orange-500 text-white dark:text-white'
        ...
        <div
          className={`absolute top-0 left-0 bottom-0 bg-orange-500 text-white dark:text-white transition-all duration-300 rounded-full ${
            theme === 'light'
              ? 'w-1/3'
              : theme === 'dark'
                ? 'w-1/3 left-1/3'
